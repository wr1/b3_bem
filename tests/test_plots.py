...
from b3_bem.plots.plots import plot_planform, plot_interpolated_polars, plot_polars, plot_bladeloads, plot_moments, rotorplot
...
def test_plot_bladeloads():
    ...
    with patch('b3_bem.plots.plots.plot_grid') as mock_grid, \
         patch('b3_bem.plots.plots.plt.savefig'):
        mock_fig = Mock()
...
def test_plot_moments():
    ...
    with patch('b3_bem.plots.plots.plt.subplots') as mock_subplots, \
        ...
        loads_list = [{'Np': np.array([1, 2]), 'Tp': np.array([0.1, 0.2])}]
        uinf_list = [5]
        moments_dict = {'flapwise': np.array([10]), 'edgewise': np.array([5]), 'combined_rms': np.array([11.2])}
...
def test_rotorplot():
    ...
    with patch('b3_bem.plots.plots.plot_grid') as mock_grid, \
        ...
        mock_grid.return_value = (mock_fig, mock_axs)
        op = {'P': [100, 200], 'CP': [0.4, 0.45], 'T': [1000, 1500], 'Mb': [10000, 12000]}
        uinf = np.array([5, 10])
        rotorplot(op, uinf)
        mock_grid.assert_called_once()
